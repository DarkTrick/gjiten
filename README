...
Gjiten uses gconf to store its settings, so you can use the gconf-editor to
poke the settings directly under apps/gjiten.

Bugs

Check for known bugs here first: http://gjiten.sourceforge.net/BUGS. If it's
not listed here and you can reproduce it then report it to me please.

Future Improvements

Check out http://gjiten.sourceforge.net for the latest release. Read the TODO
file to see what's planned for future releases. Feel free to email me with
other suggestions.

A few people have asked for a --disable-gnome configure option (that is a GTK+
only build). IMHO: if you can afford to have the GTK+ libs installed on your
system, then you can afford the Gnome libs also. The gnome libraries provide
features that would take me much longer to code in pure GTK+. I would have to
rewrite and add lots of stuff to the sources for this, and I just don't have
the time. Instead I'd like to add new features. If you want to do this, then
feel free to hack code and send a patch to me.

Feedback

Send me suggestions, wishes, comments, patches, bug reports, money [optional]
;) You can contact me at: <boti@rocketmail.com>
